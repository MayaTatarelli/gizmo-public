...
//Adding test case for my keplerian disk test problem -- MayaT
void GravAccel_KeplerianTestProblem_maya(void);
...
void GravAccel_FilamentTurbInit()
{
    ...
    int i,k; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        ...
        if (dx1>0){ //expression for the x component of the accceleration outside the cylinder (approximate)
            x_expr = STARFORGE_FILAMENT_LENGTH + sqrt(dx1*dx1 + R2) - sqrt(dx2*dx2 + R2);
            x_trunc_factor = x_expr / ( STARFORGE_FILAMENT_LENGTH + r - sqrt(STARFORGE_FILAMENT_LENGTH*STARFORGE_FILAMENT_LENGTH + R2) );
        }
...
void GravAccel_KeplerianTestProblem_maya()
{
    ...
    double r_in=0.18, r_out=2.05, r_in_true=0.2;
    printf("\n I GOT UPDATED 1! \n");
    int i; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        double r = pow(pow(P[i].Pos[1]-y00,2.)+pow(P[i].Pos[0]-x00,2.),0.5);
        double H_factor = 10.0;
        double H = H_factor*0.05*pow(r_in_true,5./4.);
